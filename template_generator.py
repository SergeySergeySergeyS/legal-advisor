...
СИТУАЦИЯ: [ДЕТАЛЬНОЕ описание 4-6 предложений от ПЕРВОГО ЛИЦА "Я приобрёл..." — ТОЛЬКО факты из вопроса!]
...
- ЗАКОНЫ: используй ТОЛЬКО те статьи, которые реально относятся к данной ситуации
...
ОТ: [если есть ФИО/адрес/телефон — укажи, иначе оставь ПУСТЫМ]
СИТУАЦИЯ: [ДЕТАЛЬНОЕ описание 4-6 предложений от ПЕРВОГО ЛИЦА — ТОЛЬКО факты из вопроса!]
ЗАКОНЫ: [ВСЕ статьи законов из ответа юриста с названиями, через точку с запятой]
...
СИТУАЦИЯ: [ДЕТАЛЬНОЕ описание 5-7 предложений от ПЕРВОГО ЛИЦА — ТОЛЬКО факты из вопроса!]
ЗАКОНЫ: [ВСЕ статьи законов из ответа юриста с названиями, через точку с запятой]
...
ОТ: [если есть ФИО/адрес/телефон — укажи, иначе оставь ПУСТЫМ]
СИТУАЦИЯ: [ДЕТАЛЬНОЕ описание 3-5 предложений от ПЕРВОГО ЛИЦА — ТОЛЬКО факты из вопроса!]
ЗАКОНЫ: [ВСЕ статьи законов из ответа юриста с названиями, через точку с запятой]
...
def analyze_chat_for_template(chat_history: list, category: str, auth_key: str, doc_type: str = "претензия") -> dict:
    ...
    try:
        ...
        # === НОВОЕ: Берём ТОЛЬКО последний вопрос пользователя и последний ответ ИИ ===
        # Это предотвращает смешение разных ситуаций
        last_user_question = ""
...
ВАЖНО: Создавай документ ТОЛЬКО на основе ПОСЛЕДНЕГО вопроса. НЕ используй информацию из предыдущих вопросов."""
...
def split_into_items(text: str) -> list:
    """Разбивает текст на пункты. УЛУЧШЕННАЯ ВЕРСИЯ: объединяет короткие строки."""
    if not text:
        return []
