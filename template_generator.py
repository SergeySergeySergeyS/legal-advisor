...
- Условная госпошлина: освобождение только для ЗоЗПП, трудовых, ГИБДД
...
Для ИМУЩЕСТВЕННЫХ исков (возмещение ущерба, взыскание денег, раздел имущества):
...
- Жилищные споры (ущерб): сумма ущерба
...
- Иски о восстановлении срока принятия наследства
...
ПРИМЕР ЦЕНЫ ИСКА для имущественного иска (каждый пункт с новой строки):
...
=== ПРАВИЛА ГОСПОШЛИНЫ (КРИТИЧЕСКИ ВАЖНО!) ===
...
- Семейные споры: развод (650 руб.), раздел имущества (% от цены иска по ст. 333.19 НК РФ)
- Наследственные споры
- Жилищные споры (включая неимущественные — 300 руб. для физлиц)

Формат записи в приложениях: "Квитанция об уплате государственной пошлины"

=== ОСВОБОЖДЕНИЕ ОТ ГОСПОШЛИНЫ ДЛЯ ЗОЗПП ===

Согласно ст. 333.36 НК РФ, потребители ОСВОБОЖДАЮТСЯ от госпошлины по искам до 1 000 000 руб.
Поэтому для исков по ЗоЗПП:
...
- СЦЕНАРИЙ А (Срок пропущен + НЕ было фактического принятия — наследник не жил в квартире, не платил ЖКУ, не забирал вещи): 
  ...
  Цена иска: "Иск неимущественного характера" (так как требования о восстановлении срока — неимущественные).
- СЦЕНАРИЙ Б (Было ФАКТИЧЕСКОЕ принятие — наследник проживал в квартире, оплачивал ЖКУ, охранял имущество, но не оформил у нотариуса): 
  ...
  Цена иска: стоимость наследуемого имущества (так как есть требование о признании права собственности).
...
- Защита прав потребителей: стоимость товара, неустойка (ст. 23 ЗоЗПП), компенсация морального вреда (ст. 15 ЗоЗПП), штраф 50% (ст. 13 п. 6 ЗоЗПП)
- Семейные споры: алименты в долях (1/4, 1/3, 1/2) или в твёрдой сумме, раздел имущества, компенсация морального вреда
- Наследство: зависит от сценария (см. выше)
- Жилищные споры: возмещение имущественного ущерба, перерасчёт, понуждение к ремонту, компенсация морального вреда, признание утратившим право пользования, выселение
- Трудовые: восстановление на работе, средний заработок за вынужденный прогул, компенсация морального вреда
...
4. Квитанция об уплате государственной пошлины (КРОМЕ исков по ЗоЗПП, трудовых и обжалования ГИБДД)
...
- Любые куски статей законов (например, "36 Налогового кодекса", "ст. 1155 ГК РФ", "333.36")
...
def is_non_property_claim(template_data: dict) -> bool:
    ...
    Для неимущественных исков НЕ нужен расчёт цены иска.
    """
    # Проверяем цену иска
    ...
    # Признаки неимущественных исков
    non_property_markers = [
        ...
        'расторгнуть брак',  # если нет раздела имущества
        'порядок общения',
        'лишить родительских',
        'признать недостойн',
    ...
    # Признаки имущественных исков (перевешивают)
    property_markers = [
        'взыскать',
        'стоимость',
        'раздел имуществ',
    ...
    # Если есть явные признаки имущественного иска — не считаем неимущественным
    has_property = any(marker in requirements for marker in property_markers)
    if has_property:
        return False
    
    # Если есть признаки неимущественного — считаем неимущественным
...
def is_fee_exempt_category(template_data: dict) -> bool:
    ...
    Освобождены: ЗоЗПП, трудовые, ГИБДД, алименты.
    """
    # Проверяем правовое обоснование и требования на признаки категории
    ...
    # Защита прав потребителей — освобождены
    if any(x in combined for x in ['зозпп', 'о защите прав потребителей', '2300-1', 'неустойк.*ст. 23']):
        return True
...
def generate_legal_document(template_data: dict, output_dir: str, doc_type: str = "претензия") -> str:
    ...
    if doc_type == "иск":
        ...
        if not is_non_property:
            # Имущественный иск — добавляем расчёт цены иска
            if 'расчёт' not in att_text and 'расчет' not in att_text:
                att_lines.append("Расчёт цены иска")
        else:
            # Неимущественный иск — удаляем расчёт цены иска, если ИИ его добавил
            att_lines = [line for line in att_lines 
